...
function BannerExecutiveProgram({ background, title, subtitle, firstItem, secondItem, downloadButton, applyButton }: Props) {
  return (
    <div class={`container-fluid bg-[#252527] md:h-[560px] xs:h-[400px] relative`}>
        <div
        ...
        <div class="absolute text-[#fefefe] lg:top-28 lg:right-[15%] md:top-[20rem] md:right-[1rem] lg:w-[30rem] xl:w-[50rem] xs:top-[12rem] xs:right-3 text-right">
            <span class="lg:text-[58px] md:text-[50px] uppercase font-AvenirNextLTPro md:w-[35rem] xs:w-[20rem] xs:text-[32px] block">{ title }</span>
            <span class="lg:text-[22px] leading-[30px] block md:w-[35rem] text-right my-3 xs:w-[20rem] xs:text-[16px]" dangerouslySetInnerHTML={{ __html: subtitle }}></span>
        </div>
          ...
          <div class="w-full xs:hidden md:flex p-2 bg-[#252527]">
              ...
              <div class="flex lg:ml-28 md:ml-10 md:text-[14px] bottom-0 relative font-bold">
                  <a href={ downloadButton.href } class="border-2 border-[#f26f21] text-[#f26f21] hover:bg-[#f26f21] hover:text-[#fefefe] py-1 md:py-2 px-4 rounded-sm uppercase">{ downloadButton.text }</a>
                  <a href={ applyButton.href } class="border-2 border-[#f26f21] text-[# bg-[#f26f21] text-[#fefefe] py-1 px-4 md:py-2 rounded-sm uppercase ml-3">{ applyButton.text }</a>
              </div>
