...
function NavItem({ item }: { item: INavItem }) {
  ...
  const toggleExpand = () => {
    if(itemIndex === displayNavbarMenu.value) displayNavbarMenu.value = undefined;
  ...
  return (
    <div class="flex mr-6 cursor-pointer h-[100%] md:relative">
      <div class="m-auto flex">
        ...
        <Icon
            id={(item.children && displayNavbarMenu.value === itemIndex) ? 'ChevronUp' : 'ChevronDown'}
      ...
      { (item.children && displayNavbarMenu.value === itemIndex) ? 
        <>
          ...
          {item.children.map(item => 
              <div class="mb-2">
                <a href={ item.href } class="text-[12px] uppercase text-[#101010] border-b-[#475769] border-b-[1px] hover:border-b-[2px] font-medium outline-none">{ item.label }</a>
                <p class="text-[#848b94] text-[12px] my-1 mb-1"> { item.description } </p>
                
                <div class="ml-2 my-4">

                  {item.children?.map(link =>
                    <a style={buttonArrow} href={ link.href } class="text-[#505050] hover:text-[#202020] text-[12px] border-b-[#eaeaea] border-b-[1px] w-[90%] pb-2 mb-2 block bg-[length:15px] bg-[99%_20%] hover:bg-[url(/icons/Arrow-right-black.svg)]">
